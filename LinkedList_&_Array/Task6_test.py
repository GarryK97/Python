...
def convert_line_toindex_test():
    ...
    test1_file.textlist = test1_file.linkedlist_readfile("test1.txt")  # so, test1_file = [0,1,2,3,4,5] in list format
...
def insert_test():
    ...
    test1_file.textlist = test1_file.linkedlist_readfile("test1.txt")  # Resets test1_file, because it is modified in above test
...
def read_test():
    ...
    test2_file.textlist = test2_file.linkedlist_readfile("test2.txt")  # so, test2_file = [1,3,5,7,9] in list format

    test3_file = Editor()  # test3.txt has 11,22,33,44,55 in separated line
    test3_file.textlist = test3_file.linkedlist_readfile("test3.txt")  # so, test3_file = [11,22,33,44,55] in list format
...
def write_test():
    ...
    real_test.textlist = real_test.linkedlist_readfile("real_test.txt")  # Reading the file again, to check
    ...
    real_test.textlist = real_test.linkedlist_readfile("real_test.txt")
...
def search_test():
    ...
    test1_file.textlist = test1_file.linkedlist_readfile("test_search1.txt")

    test2_file = Editor()  # test_search2.txt has (A, FOUN, A, foun, FoUnD)
    test2_file.textlist = test2_file.linkedlist_readfile("test_search2.txt")

    test3_file = Editor()  # test3.txt has [A, foun, F'oun,d, F..ound, A, f/o,und] (to check ignoring special characters)
    test3_file.textlist = test3_file.linkedlist_readfile("test_search3.txt")
...
def test_functions():
    ...
    search_test()
    """
    1. I did not Include test for linkedlist_readfile() because I used the function from Task4, so the test is included in task4_test
    2. Also, I did not test quit() because it only uses the python built-in function exit().
    3. All of them has no precondition on LinkedList functions because it can be tested in Task3_test, so I assumed that they work properly
    """
