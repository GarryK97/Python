...
class LinkedList:
    ...
    def insert(self, index, item):
        ...
        if index == len(self):  # if user wants to insert the item at the last,
            ...
            raise IndexError("Index is out of boundary")
        elif index == 0 or index == -len(self):  # if index = 0, it needs to be treated differently to prevent error, e.g.) self.get_node(0-1) will give the last node
            self.head = Node(item, self.head)  # Sets start node to new node and links the new node to original start node.
            self.count += 1  # Updates the length
        else:
