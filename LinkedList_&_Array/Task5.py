...
class Editor:
    def __init__(self):
        ...
        """
        self.textlist = Array_Based_List()  # Initializing list from Task2

    def arraylist_readfile(self, filename):
        """
        Reads a file and make a Array_Based_List class object (Copied from Task4)
    ...
    def read(self, filename):
        ...
        :precondition: arraylist_readfile() must have no error
        :complexity: O(N), including arraylist_readfile()
        """
        try:
            self.textlist = self.arraylist_readfile(filename)  # textlist now has the texts of the file
            return True
    ...
    def delete(self, line_num=None):
        ...
        if line_num is None:  # If no number given,
            self.textlist = Array_Based_List()  # Reset
            return True
