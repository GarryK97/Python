...
class Array_Based_List:
    ...
    def __init__(self, size):
        ...
        :complexity: O(1)
        """
        assert isinstance(size, int), "size argument must be int type"  # Prevents non-int type coming in
        assert size > 0, "Size of the array must be bigger than 0"  # if size is negative or 0, raise error
        assert size <= 50, "Maximum size of an array is 50"  # Maximum size is set to 50 (Assignment2 Rule)
        self.array = build_array(size)  # Make an array, using imported library, build_array(size)
        self.count = 0  # counts the number of elements of the array. (Simply, the real length of the array)
        self.size = size  # Saves the total size of the array
    ...
    def append(self, item):
        ...
        :precondition: is_full(), __setitem__ must have no error
        :complexity: O(1)
        """
        if self.is_full():
            raise Exception("The array is full, cannot append the item")  # if full, raise Error
        else:
    ...
    def insert(self, index, item):
        ...
        :precondition: is_full(), is_index_inbound(), __len__, __setitem__, __getitem__ must have no error
        :complexity: O(N)
        """
        assert isinstance(index, int), "insert(index, item): index must be int type"  # Checks if the index is int type
        if self.is_full():
            raise Exception("The array is full, cannot insert the item")  # if the list is full, raise error
        elif index == len(self):  # If user wants to insert the item after last element,
    ...
    def remove(self, item):
        ...
        :precondition: __len__, __getitem__, __setitem__ must have no error
        :complexity: O(N^2)
        """
        for i in range(len(self)):
    ...
    def delete(self, index):
        ...
        :precondtion: is_index_inbound(), __len__, __getitem__, __setitem__ must have no errors
        :complexity: O(N)
        """
    ...
    def copy(self):
        ...
        :complexity: O(N)
        """
        copylist = Array_Based_List(self.size)
        for i in range(len(self)):
